...
public interface PassRepository extends JpaRepository<Pass, Long> {
    ...
    @Query(value="select * from pass p where p.is_pass_active <> 'FALSE' and p.pass_id not in (select p.pass_id from pass p inner join loan_pass lp on lp.pass_id = p.pass_id inner join loan l on lp.loan_id = l.loan_id where l.loan_status <> 'canceled' and l.loan_date = :loanDate)", nativeQuery=true)
    List<Pass> findAvailablePassesForADate(@Param("loanDate") String loanDate);

    @Query(value="select * from pass p where p.is_pass_active <> 'FALSE' and p.pass_type = :passType and p.pass_id not in (select p.pass_id from pass p inner join loan_pass lp on lp.pass_id = p.pass_id inner join loan l on lp.loan_id = l.loan_id where l.loan_status <> 'canceled' and l.loan_date = :loanDate)", nativeQuery=true)
    List<Pass> findAvailablePassesForPassTypeAndDate(@Param("passType") String passType, @Param("loanDate") String loanDate);

    @Query("SELECT DISTINCT l from Loan l JOIN l.passList p where p = :pass AND l.loanDate = :loanDate AND l.loanStatus <> 'canceled'")
    Loan findLoanForAPassAndDate(@Param("pass") Pass pass, @Param("loanDate") String loanDate);

    @Query("SELECT DISTINCT p from Pass p JOIN p.loans l where l.loanDate = :loanDate and l.loanStatus <> 'canceled'")
